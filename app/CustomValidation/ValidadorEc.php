...
class ValidadorEc
{
    ...
    {
        if (empty($numero)) {
            throw new Exception(__('messages.identification-empty'));
        }

        if (!ctype_digit($numero)) {
            throw new Exception(__('messages.identification-integer'));
        }

        if (strlen($numero) != $caracteres) {
            throw new Exception(__('messages.identification-between', ['number' => $caracteres]));
        }
    ...
    protected function validarCodigoProvincia($numero)
    {
        if ($numero < 0 || $numero > 24) {
            throw new Exception(__('messages.identification-failed'));
        }
    ...
    protected function validarTercerDigito($numero, $tipo)
    {
        switch ($tipo) {
            ...
            case 'ruc_publica':
                ...
                break;
            default:
                throw new Exception(__('messages.identification-not-exists'));
                break;
        }
