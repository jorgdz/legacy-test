...
class SubjectCurriculumRepository extends BaseRepository
{
     ...
     * @return void
     */
    public function __construct (SubjectCurriculum $matterMesh) {
     ...
     * showDependencies
     *
     * @param  mixed $SubjectCurriculum
     * @return void
     */
    public function showDependencies(SubjectCurriculum $matterMesh) {
    ...
    public function setMatterMesh($id, $old, $new) {
        $order = SubjectCurriculum::findOrFail($id);

        if ($new > $old) {
            SubjectCurriculum::WhereBetween('order', [(int)$old, (int)$new])->decrement('order');
            $order->order = $new;

            return $order->save();
        }
        
        SubjectCurriculum::WhereBetween('order', [(int)$new, (int)$old])->increment('order');
        $order->order = $new;
     ...
     * @return void
     */
    public function showPrerequisites(SubjectCurriculum $matterMesh) {
